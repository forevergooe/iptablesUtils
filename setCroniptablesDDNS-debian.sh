...
# wget https://raw.githubusercontent.com/forevergooe/iptablesUtils/master/setCroniptablesDDNS.sh;bash setCroniptablesDDNS.sh
...
wget https://raw.githubusercontent.com/forevergooe/iptablesUtils/master/iptables4ddns.sh  &> /dev/null
...
echo "rm -f /root/$IPrecordfile" >> /etc/rc.local
# 替换下面的localport remoteport targetDDNS
echo "/bin/bash /usr/local/iptables4ddns.sh $localport $remoteport $targetDDNS $IPrecordfile &>> /root/iptables${localport}.log" >> /etc/rc.local
chmod +x /etc/rc.local
# 定时任务，每分钟检查一下
